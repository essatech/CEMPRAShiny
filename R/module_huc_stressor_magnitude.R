  ...
  moduleServer(id,
               function(input, output, session) {
                 ...
                 output$stressor_inputs <- renderDT({
                   ...
                   # Get ALL stressor names from stressor response workbook
                   # This ensures all stressors are shown even if data is missing for a location
                   all_stressors <- isolate(session$userData$rv_stressor_response$stressor_names)
                   ...
                   if(length(selected_ids) > 0) {
                     ...
                     # If no stressor magnitude data exists for selected locations,
                     # still show all stressors with NA values
                     if(nrow(dr) == 0) {
                       # Set placeholder CSC values to NA
                       session$userData$rv_clickedIds_csc$csc <- NA
                       session$userData$rv_clickedIds_csc$var_csc <- data.frame(
                         snames = all_stressors,
                         sys_cap = NA
                       )
                     } else {
                   ...
                   if (length(selected_ids) == 1) {
                     ...
                     huc_data <- raw_data %>% filter(HUC_ID == selected_ids)

                     # Create a dataframe with ALL stressors, then merge with available data
                     # This ensures all stressors are shown even if data is missing
                     table_vals <- data.frame(Stressor = all_stressors)
                     table_vals <- base::merge(table_vals, huc_data,
                                         by = "Stressor",
                   ...
                   if (length(selected_ids) > 1) {
                     ...
                     # Use ALL stressors from stressor response workbook
                     # This ensures all stressors are shown even if some locations have missing data
                     table_vals <-
                       data.frame(
                 ...
                 observeEvent(input$stressor_inputs_cell_edit, {
                   ...
                   # Get ALL stressor names from stressor response workbook
                   # Use this instead of filtering from magnitude data to handle missing stressors
                   all_stressors <- isolate(session$userData$rv_stressor_response$stressor_names)
                   snames <- sort(all_stressors)
                   ...
                   if (info$value == "") {
                     ...
                     if (info$value == "normal") {
                       ...
                       # Check if this stressor exists for this HUC in the magnitude data
                       # If not, we need to add a new row
                       existing_row <- which(
                       ...
                       if (length(existing_row) > 0) {
                         # Update existing row
                         session$userData$rv_stressor_magnitude$sm_dat[existing_row, var[j + 1]] <- info$value
                       } else {
                         # Need to add new rows for each selected HUC
                         # Get the column names from the existing dataframe to ensure compatibility
                         existing_cols <- colnames(session$userData$rv_stressor_magnitude$sm_dat)

                         for (huc_id in selected_ids) {
                           # Create a new row with all columns from existing data, set to NA
                           new_row <- as.data.frame(matrix(NA, nrow = 1, ncol = length(existing_cols)))
                           colnames(new_row) <- existing_cols

                           # Set the required values
                           new_row$HUC_ID <- huc_id
                           new_row$Stressor <- snames[i]
                           if ("SD" %in% existing_cols) new_row$SD <- 0
                           if ("Distribution" %in% existing_cols) new_row$Distribution <- "normal"

                           # Set the edited value
                           new_row[, var[j + 1]] <- info$value
