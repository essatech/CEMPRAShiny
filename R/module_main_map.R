      ...
      r_huc_polygons <- reactive({
        ...
        if (var_name != "system_capacity") {
          ...
          if (is.null(resp_curv)) {
            ...
            sys_cap_resp <- CEMPRA::interaction_matrix_sys_cap(MInt = MInt, sc.dose.df = sm_df, adult_sys_cap = FALSE)
            sys_cap_resp <- sys_cap_resp[which(sys_cap_resp$Stressor == var_name), ]
      ...
      output$stressor_variable_list <- renderUI({
        ...
        if (!(is.null(mnames))) {
          for (m in 1:length(mnames)) {
            ...
            svar_list[[var_id]] <- module_stressor_variable_ui(ns(this_interaction))
