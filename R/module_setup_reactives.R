module_setup_reactives <- function(session = NA) {
  ...
  rv_stressor_magnitude <- reactiveValues(sm_dat = sm_wb_dat)
  ...
  rv_show_pop_main_plot <- reactiveValues(open = FALSE)
  ...
  rv_HUC_geom <- reactiveValues(
    huc_geom = hmdl,
    # Polygon geometry global reactive object
    leg_col = NA,
  ...
  rv_redraw <- reactiveValues(redraw = 0)
  ...
  rv_joe_model_results <- reactiveValues(sims = list())
  session$userData$rv_joe_model_results <- rv_joe_model_results
  
  
  # Joe model results holder - assume multiple runs
  rv_joe_model_results_scenarios <- reactiveValues(sims = list())
  session$userData$rv_joe_model_results_scenarios <-
    rv_joe_model_results_scenarios
  
  
  # Joe model scenario name holder - assume multiple runs
  rv_joe_model_sim_names <- reactiveValues(scenario_names = list())
  session$userData$rv_joe_model_sim_names <- rv_joe_model_sim_names
  
  
  # Place holder for Joe Model estimated run times
  rv_joe_model_run_time <- reactiveValues(run_time_seconds = list())
  ...
  session$userData$rv_se_inputs <- rv_se_inputs
